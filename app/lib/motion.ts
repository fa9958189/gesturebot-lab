...
function easyMotion(value: number, fingerIndex: number) {
  const start = fingerIndex === 0 ? 0.02 : 0.035;
  const end = fingerIndex === 0 ? 0.56 : 0.64;
...
export function fingerCurls(landmarks: NormalizedLandmark[]) {
  return FINGER_JOINTS.map(([mcp, pip, dip, tip], index) => {
    ...
    const combined = index === 0
      ? angleCurl * 0.6 + reachCurl * 0.4
      : angleCurl * 0.7 + reachCurl * 0.3;
    return easyMotion(combined, index);
  });
}
...
export function poseFromHand(landmarks: NormalizedLandmark[], curls: number[]): MotionPose {
  ...
  return {
    ...
    leftShoulder: 3 + index * 76,
    leftElbow: index * 88,
    rightShoulder: -3 - ring * 76,
    rightElbow: -ring * 88,
    leftHip: thumb * 27,
    leftKnee: -thumb * 62,
    rightHip: -pinky * 27,
...
export function smoothPose(current: MotionPose, target: MotionPose) {
  ...
  const blend = clamp(0.2 + largestDelta / 145, 0.2, 0.48);
  const next = { ...current };
  for (const key of Object.keys(target) as (keyof MotionPose)[]) {
